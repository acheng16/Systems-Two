...
void set_history(FILE *fo, int count) {
	...
	fclose(fo);                
	fo = fopen("history.txt","r");         //reopens the history text file to make pointer point to the start of the file
	...
	fclose(fo);
	fclose(temp_file);
 	rename("temp.txt","history.txt");   //replace history file with temp file.
  	fo = fopen("history.txt","a");
	
}

/* searches through the file if the command entered exists */
void line_search(FILE *fo, char inBuffer[]){
...
int retrieve_from_history(char inBuffer[], FILE *fo, int r_num, char *args[],int *bkgrnd, int line_number) {
  char line[MAXLINE];
...
void setup(char inBuffer[], char *args[],int *bkgrnd, FILE *fo)
{
...
int main(void)
{
	...
	printf("SysIIShell--> ");  /* Shell prompt */
        fflush(0);
        setup(inBuffer, args, &bkgrnd, file_history);    /* Get next command */
...
if(args[0]!=NULL) {
	...
	//case 2 r entered and args[1] exists
		...
		if (digit<0){
		  ...
		  bool = 1;
			...
			if (!retrieve_from_history(inBuffer, file_history, digit, args, &bkgrnd, line_number)) {
	...
	if ( strcmp (args[0], "rr") == 0 && args[1] == NULL ) {
		if (line_number==1) {
			...
			retrieve_from_history(inBuffer, file_history, line_number-1, args, &bkgrnd, line_number);
		}
	}
	...
	if ( strcmp (args[0], "sethistory") == 0 && args[1] != NULL) {
		...
		if (digit!=0){
			set_history(file_history,set_history_num);
		}else {
